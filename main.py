...
def safe_update_gui(name, target, actual):
    if gui and gui.root:
        gui.root.after(0, gui.update_dashboard_value, name, target, actual)
...
def control_loop():
    ...
    while RUNNING:
        ...
        # === Dashboard Update ===
        safe_update_gui("Velocity", pid_manager.pid_velocity_to_tilt_angle.target_velocity, 0.0)
        safe_update_gui("Tilt Angle", pid_manager.pid_tilt_angle_to_torque.target_angle, estimated_tilt_angle)
        safe_update_gui("Torque", pid_manager.pid_estimated_torque_to_torque.target_torque, 0.0)
        safe_update_gui(
            "Z Angular Velocity",
            pid_manager.pid_angular_velocity_to_torque_differential.target_torque,
            0.0 if not global_config.enable_yaw_loop else imu.read_gyro_z()
        )
